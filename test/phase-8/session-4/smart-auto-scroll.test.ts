...
describe('Session 4: Smart Auto-Scroll', () => {
  ...
  test('source uses explicit user-intent and programmatic-scroll guards', async () => {
    ...
    expect(source).toContain('captureLockedViewportAnchor')
    expect(source).toContain('restoreViewportAnchor')
    expect(source).toContain('lockViewport={sessionAgentSdk === \'codex\' && showScrollFab}')
